...
static heap_t*
heap_allocate(rpmalloc_interface_t* memory_interface, int first_class);
...
get_thread_heap_allocate(void) {
	heap_t* heap = heap_allocate(global_memory_interface, 0);
...
static void*
os_mmap(void *unused, size_t size, size_t alignment, size_t* offset, size_t* mapped_size) {
	...
	if (!ptr) {
		if (global_memory_interface->map_fail_callback) {
			if (global_memory_interface->map_fail_callback(global_memory_interface->context, map_size))
				return os_mmap(NULL, size, alignment, offset, mapped_size);
		} else {
...
static void
os_mcommit(void *unused, void* address, size_t size) {
#if ENABLE_DECOMMIT
...
static void
os_mdecommit(void *unused, void* address, size_t size) {
#if ENABLE_DECOMMIT
	if (global_config.disable_decommit)
		return;
#if PLATFORM_WINDOWS
...
static void
os_munmap(void *unused, void* address, size_t offset, size_t mapped_size) {
...
page_decommit_memory_pages(page_t* page) {
	...
	page->heap->memory_interface->memory_decommit(page->heap->memory_interface->context, extra_page, extra_page_size);
...
page_commit_memory_pages(page_t* page) {
	...
	void* extra_page = pointer_offset(page, global_config.page_size);
	size_t extra_page_size = page_get_size(page) - global_config.page_size;
	page->heap->memory_interface->memory_commit(page->heap->memory_interface->context, extra_page, extra_page_size);
...
span_allocate_page(span_t* span) {
	...
	if (span->page_initialized)
		heap->memory_interface->memory_commit(heap->memory_interface->context, page, span->page_size);
#endif
...
span_deallocate_block(span_t* span, page_t* page, void* block) {
	if (UNEXPECTED(page->page_type == PAGE_HUGE)) {
		span->heap->memory_interface->memory_unmap(span->heap->memory_interface->context, span, span->offset, span->mapped_size);
		return;
	}
...
static heap_t*
heap_allocate_new(rpmalloc_interface_t* memory_interface) {
	...
	if (!memory_interface)
	    memory_interface = global_memory_interface;
	block_t* block = memory_interface->memory_map(memory_interface->context, heap_size, 0, &offset, &mapped_size);
	if (!block)
	    return NULL;
#if ENABLE_DECOMMIT
	memory_interface->memory_commit(memory_interface->context, block, heap_size);
#endif
...
#endif
    heap->memory_interface = memory_interface;
	return heap;
}

static void
heap_unmap(heap_t* heap) {
	heap->memory_interface->memory_unmap(heap->memory_interface->context, heap, heap->offset, heap->mapped_size);
}
...
heap_allocate(rpmalloc_interface_t* memory_interface, int first_class) {
	...
	if (!heap)
		heap = heap_allocate_new(memory_interface);
	if (heap) {
...
heap_get_span(heap_t* heap, page_type_t page_type) {
	...
	size_t offset = 0;
	size_t mapped_size = 0;
	span_t* span = heap->memory_interface->memory_map(heap->memory_interface->context, SPAN_SIZE, SPAN_SIZE, &offset, &mapped_size);
	if (EXPECTED(span != 0)) {
...
#if ENABLE_DECOMMIT
		heap->memory_interface->memory_commit(heap->memory_interface->context, span, page_size);
#endif
		span->heap = heap;
...
heap_allocate_block_huge(heap_t* heap, size_t size, unsigned int zero) {
	...
	size_t offset = 0;
	size_t mapped_size = 0;
	void* block = heap->memory_interface->memory_map(heap->memory_interface->context, alloc_size, SPAN_SIZE, &offset, &mapped_size);
	if (block) {
		span_t* span = block;
#if ENABLE_DECOMMIT
		heap->memory_interface->memory_commit(heap->memory_interface->context, span, alloc_size);
#endif
		span->heap = heap;
...
heap_free_all(heap_t* heap) {
	for (int itype = 0; itype < 3; ++itype) {
		...
		while (span) {
			...
			heap->memory_interface->memory_unmap(heap->memory_interface->context, span, span->offset, span->mapped_size);
			span = span_next;
		}
...
rpmalloc_heap_t*
rpmalloc_heap_create(rpmalloc_interface_t* memory_interface) {
	...
	heap_t* heap = heap_allocate(memory_interface, 1);
	if (!heap)
	    return NULL;
...
void
rpmalloc_heap_destroy(rpmalloc_heap_t* heap) {
	if (heap)
		heap_unmap(heap);
}
